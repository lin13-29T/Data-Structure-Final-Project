...
public class PosOrderIterator<E> implements ITreeIterator<E> { 
	...
	public PosOrderIterator(Tree<E> tree) { 
		...
		if(!tree.isEmpty())   
			this.nextNode = new 
			StackNode<E>(moveCursorToLastLeftOrRightNode((BinaryTreeNode<E>)
					tree.getRoot())); 
	...
	public BinaryTreeNode<E> nextNode() { 
		...
		if(nextNode != null){ 
			...
			if(nextNode.getRight() != null && 
				...
				nextNode = new 
						StackNode<E>(moveCursorToLastLeftOrRightNode(nextNode.getRight()))
						; 
			} 
				...
				if(!stack.isEmpty()){      
					...
					if(father.getCount() == 1 && 
							father.getRight() != null){ 
						father.incrementCount(); 

						stack.push(father); 

						nextNode = new 
								StackNode<E>(moveCursorToLastLeftOrRightNode(father.getRight())); 
					}  
				} 
			} 
		} 
	...
	moveCursorToLastLeftOrRightNode(BinaryTreeNode<E> initialNode){ 
		...
		if(cursor.getRight() != null){ 
			...
			cursor = 
					moveCursorToLastLeftOrRightNode(cursor.getRight()); 
		} 
