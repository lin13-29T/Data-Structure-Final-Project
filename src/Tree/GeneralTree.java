...
public class GeneralTree<E> extends Tree<E> implements Serializable {  
	...
	// Metodo 4 de la cp 12
	 public GeneralTree(List<Tree<E>> trees){
		if(trees!=null && !trees.isEmpty()){
			this.root = trees.get(0).getRoot();
			if(trees.size() > 1){
				BinaryTreeNode<E> convert = (BinaryTreeNode<E>)this.root;
				for(int i=0;i<trees.size();i++){
					BinaryTreeNode<E> treeRoot = (BinaryTreeNode<E>)trees.get(i).getRoot();
	...
	public BinaryTreeNode<E> getFather(BinaryTreeNode<E> node) { 
		...
		if (node != null && !isEmpty() || !root.equals(node)) { 
			...
			while(iterator.hasNext() && !foundedNode){ 
				...
				if (node.equals(((BinaryTreeNode<E>) 
						cursor).getLeft())){  
					father = cursor; 
					foundedNode = true; 
				} 
...
cursor).getRight())){         
	...
	public List<TreeNode<E>> getLeaves() { 
		ArrayList<TreeNode<E>> leavesList = new 
		...
		if (!isEmpty()) {        
			...
			while(iterator.hasNext()){ 
				...
				if(((BinaryTreeNode<E>) node).getLeft() == 
						null) 
					leavesList.add(node); 
			}       
		} 
	...
	node) { 
		List<BinaryTreeNode<E>> sonsList = new 
	...
	public boolean nodeIsLeaf(TreeNode<E> node) { 
		if(node != null) 
			return ((BinaryTreeNode<E>)node).getLeft() == 
			null; 
	...
	public InBreadthIteratorWithLevels<E> 
	...
	public int treeHeight() { 
		int height = -1; 
		InBreadthIteratorWithLevels<E> iter = 
				inBreadthIteratorWithLevels(); 
